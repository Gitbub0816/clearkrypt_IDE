...
describe('emitKotlin determinism', () => {
  it('produces byte-identical output across repeated emits of the same project', () => {
    const project = irSamples.sampleTypesProject();
    const first = emitKotlin(project, { compilerVersion: '0.1.0' });
    const second = emitKotlin(project, { compilerVersion: '0.1.0' });
  ...
  it('never emits trailing whitespace and always ends with exactly one newline', () => {
    ...
    const result = emitKotlin(project, { compilerVersion: '0.1.0' });
  ...
  it('sorts generated files by path and imports within a file', () => {
